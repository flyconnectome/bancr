# Authors and Citation

## Authors

- **Alexander Bates**. Author, maintainer.
  [](https://orcid.org/0000-0002-1195-0445)

- **Gregory Jefferis**. Author.
  [](https://orcid.org/0000-0002-0587-9355)

## Citation

Source:
[`DESCRIPTION`](https://github.com/flyconnectome/bancr/blob/main/DESCRIPTION)

Bates A, Jefferis G (2025). *bancr: R Client Access to the Brain And
Nerve Cord (BANC) Dataset*. R package version 0.2.2,
<https://github.com/flyconnectome/bancr>.

    @Manual{,
      title = {bancr: R Client Access to the Brain And Nerve Cord (BANC) Dataset},
      author = {Alexander Bates and Gregory Jefferis},
      year = {2025},
      note = {R package version 0.2.2},
      url = {https://github.com/flyconnectome/bancr},
    }
